...
func (tuplet IPFlowTuplet) hashV4Flow() uint32 {
	flowHash := (ip2int(tuplet.SrcIP) * 59) ^ (ip2int(tuplet.DstIP)) ^ (uint32(tuplet.SrcPort) << 16) ^ uint32(tuplet.DstPort) ^ uint32(tuplet.Proto)
...
func getPacketInfo(packet gopacket.Packet, warn chan IPFlowTuplet) {
	...
	hash := pinfo.hashV4Flow()

	activeConnsLock.RLock()
	conn, ok := activeConns[hash]
	activeConnsLock.RUnlock()

	if !ok {
		log.Println("new connection: ", pinfo.Proto, " - ", pinfo.SrcMAC, "/", pinfo.DstMAC, " - ", pinfo.SrcIP, "/", pinfo.DstIP, " - ", pinfo.SrcPort, "/", pinfo.DstPort)
		newconn := make(chan gopacket.Packet)
		go checkConnection(newconn, warn, pinfo)

		activeConnsLock.Lock()
		activeConns[hash] = newconn
...
func checkConnection(conn chan gopacket.Packet, warn chan IPFlowTuplet, pinfo IPFlowTuplet) {
	...
	defer func() {
		activeConnsLock.Lock()
		delete(activeConns, pinfo.hashV4Flow())
		activeConnsLock.Unlock()
	}()
...
func flowClient(warn chan IPFlowTuplet) {
	for pinfo := range warn {
		log.Println("Warning: ", pinfo.Proto, " - ", pinfo.SrcMAC, "/", pinfo.DstMAC, " - ", pinfo.SrcIP, "/", pinfo.DstIP, " - ", pinfo.SrcPort, "/", pinfo.DstPort)
	}
}
